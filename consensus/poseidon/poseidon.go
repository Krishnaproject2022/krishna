...
var (
	...
	RateBlockReward = big.NewInt(60) //0.6*burn
	BaseBlockReward = big.NewInt(100)
	MaxBlockReward  = new(big.Int).Mul(big.NewInt(1), ether)

	SupplyAccount  = common.HexToAddress(systemcontracts.ValidatorFactoryContract)
	SupplyKeyHash  = common.HexToHash("0x201f1e1d1c1b1a191817161514131211100f0e0d0c0b0a090807060504030201")
	MaxTotalSupply = new(big.Int).Mul(big.NewInt(26020000), ether) //8688w - 6086w
)
...
func (c *Poseidon) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs []*types.Transaction,
	uncles []*types.Header) {
	accumulateRewards(chain.Config(), state, header, txs)
...
func accumulateRewards(config *params.ChainConfig, state *state.StateDB, header *types.Header, txs []*types.Transaction) {
	if config.IsLondon(header.Number) == false || len(txs) <= 1 {
		return
	}
	supplyHash := state.GetState(SupplyAccount, SupplyKeyHash)
	totalSupply := new(big.Int).SetBytes(supplyHash[:])

	if totalSupply.Cmp(MaxTotalSupply) >= 0 {
		return
	}

	burnFee := new(big.Int).Mul(new(big.Int).SetUint64(header.GasUsed), header.BaseFee)
	blockReward := new(big.Int).Mul(burnFee, RateBlockReward)
	blockReward = blockReward.Div(blockReward, BaseBlockReward)
	blockReward = math.BigMin(blockReward, MaxBlockReward)

	totalSupply = totalSupply.Add(totalSupply, blockReward)
	if totalSupply.Cmp(MaxTotalSupply) > 0 {
		blockReward = new(big.Int).Sub(totalSupply, MaxTotalSupply)
		totalSupply = new(big.Int).Set(MaxTotalSupply)
	}

	state.AddBalance(header.Coinbase, blockReward)
	state.SetState(SupplyAccount, SupplyKeyHash, common.BytesToHash(totalSupply.Bytes()))
}
